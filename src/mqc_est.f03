...
!PROCEDURE MQC_Matrix_SpinBlockGHF
      subroutine mqc_matrix_spinBlockGHF(array,nalpha)
!
...
!
          print *, 'in test'
          call array%mput(tmpMatrix%mat([1,rows],[1,nalpha]),[1,rows],[1,nalpha])
          call array%mput(tmpMatrix%mat([1,rows],[nalpha+1,nalpha*2]),[1,rows],[(columns/2)+1,(columns/2)+nalpha])
!          if((columns - (nalpha*2)).gt.0) then
!            call array%mput(tmpMatrix%mat([1,rows],[(nalpha*2)+1,(nalpha*2)+((nalpha*2)-(columns/2))]), &
!              [1,rows],[nalpha+1,((nalpha+1)+(columns/2))])
!          end if
