...
Source authority: [`src/mqc_est.F03`](../../src/mqc_est.F03) and
[`src/mqc_nef.F03`](../../src/mqc_nef.F03).
...
- `mqc_twoERIs` stores spin-resolved rank-4 tensors with integral-type,
  interaction-type, and storage-type metadata. Use `getBlock`, `blockSize`,
  `type`, `intType`, `storType`, `at`, `interaction`, and `print` rather than
  reaching into private spin blocks. Storage values are `full` or `symm`;
  interaction values include `coulomb`, `exchange`, `inverted`, `doublebar`,
  and `ms`.
- `mqc_twoERISet` owns multiple two-electron-integral objects and provides
  `erinum`, `eris`, `transform`, `addtoset`, and `print`.

### Natural Excitation Framework objects

`MQC_NEF` builds on `MQC_EST` and exposes `MQC_NEF_Vector`,
`MQC_NEF_Matrix`, and `MQC_NEF_R4`. These objects store block-partitioned EST
vectors, matrices, and two-electron-integral tensors together with mapping and
per-space dimension metadata. Use their public initialization, block/map,
dimension, import/export, slice/collapse, and element-access bindings instead
of their private backing arrays.

The module extends `MatMul`, `Dagger`, `Conjg`, `Transpose`, `Contraction`,
`PartialContraction`, `Size`, arithmetic operators, and NEF-specific outer and
partial-contraction operations. Exact supported operand pairings are listed in
the generated procedure index and the interface blocks in `src/mqc_nef.F03`.
